...
const ProductFormPage = () => {
    ...
    useEffect(() => {
        const fetchDependencies = async () => {
            try {
                ...
                // If edit mode, fetch the specific product
                if (id) {
                    const pRes = await api.get(`/products/${id}`);
                    const mappedData = { ...pRes.data };
                    // Map boolean back to string for select component
                    ...
                    setInitialData(null); // Explicit null for Create mode
                }
            } catch (err) {
    ...
    const handleSubmit = async (formData) => {
        ...
        // Transform numeric fields for API
        const payload = {
            ...formData,
            party_price: formData.party_price ? parseFloat(formData.party_price) : null,
        ...
        try {
            ...
            // Navigate back to the list after a brief delay so they can see success
            setTimeout(() => {
                navigate('/products');
            }, 1000);
        } catch (err) {
