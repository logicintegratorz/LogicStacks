...
const Categories = () => {
  ...
  // formData now includes is_active so it's always explicitly controlled
  ...
  const handleEdit = (cat) => {
    setEditId(cat.id);
    // Pre-fill ALL fields including is_active — prevents status from resetting
    setFormData({
      name: cat.name || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editId) {
        // Send is_active explicitly so backend preserves the correct value
    ...
    <div className="animate-fade-in" style={{ backgroundColor: '#fff', borderRadius: '8px', padding: '30px', boxShadow: '0 2px 4px rgba(0,0,0,0.02)', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '40px' }}>
      ...
      <div>
        <h2 style={{ fontSize: '20px', marginBottom: '20px', color: '#2d3748' }}>
          {editId ? '✏️ Edit Category' : 'Manage Categories'}
        </h2>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          {/* Explicit status toggle — only shown during edit */}
          {editId && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px', background: '#f7fafc', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
              <label style={{ fontWeight: '600', color: '#4a5568', fontSize: '14px', margin: 0 }}>Status:</label>
              <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontSize: '14px' }}>
                ...
                />
                <span style={{ color: formData.is_active ? '#38a169' : '#e53e3e', fontWeight: '600' }}>
          ...
          <tbody>
            {categories.map(cat => (
              <tr key={cat.id} style={{ borderBottom: '1px solid #edf2f7' }}>
                ...
                <td style={{ padding: '12px', textAlign: 'center' }}>
                  <span style={{
                    backgroundColor: cat.is_active ? '#c6f6d5' : '#fed7d7',
                    color: cat.is_active ? '#276749' : '#c53030',
